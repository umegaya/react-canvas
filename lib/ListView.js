...
var ListView = React.createClass({
  ...
  propTypes: {
    ...
    onRenderRefresh: React.PropTypes.func,
  ...
  indexOfIncludeHeight: function (height) {
    var h = 0, i = 0;
    while (true) {
      if (this.state.scroll.cachedHeights[i]) {
  ...
  render: function () {
    ...
    if (this.state.scroll.scrollTop < 0 && this.props.onRenderRefresh != null) {
      items.splice(0, 0, this.props.onRenderRefresh(this.state.scroll.scrollTop));
    }
    return (
  ...
  getVisibleItemIndexes: function () {
    ...
    var itemScrollTop = 0;
    var startIndex = this.indexOfIncludeHeight(scrollTop - this.props.style.height);
    var prevHeight = this.heightOf(startIndex) - scrollTop;
    ...
    for (var index=startIndex; index < itemCount; index++) {
      ...
      if (itemScrollTop >= this.props.style.height) {
        //one big element covers all screen
        if (prevHeight < 0 && index > 0) {
          itemIndexes.push(index - 1);
        }
        break;
      }
